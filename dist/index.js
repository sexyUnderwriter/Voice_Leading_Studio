...
function pickPdfCommand(explicitCmd) {
    ...
    throw new Error("No custom PDF renderer command found. Pass --pdf-cmd <command> or set PDF_RENDER_CMD.");
}
function exportMusicXmlToPdf(xmlPath, pdfPath, explicitCmd) {
...
const STEPS_IN_ORDER = ["C", "D", "E", "F", "G", "A", "B"];
...
function expectedAlterForStep(step, fifths) {
    if (fifths > 0)
        return SHARPS_ORDER.slice(0, fifths).includes(step) ? 1 : 0;
    if (fifths < 0)
        return FLATS_ORDER.slice(0, -fifths).includes(step) ? -1 : 0;
    return 0;
}
/** The step letter that is N diatonic steps above a given bass step (N=1 is the unison). */
function diatonicStepAbove(bassStep, n) {
...
function toFigAccidental(acc, expectedAlt) {
    if (acc === 0)
        return undefined; // in key
    if (acc > 0 && expectedAlt < 0)
        return "natural"; // cancelling a flat
    if (acc < 0 && expectedAlt > 0)
        return "natural"; // cancelling a sharp
    if (acc === 1)
        return "sharp";
    if (acc === -1)
        return "flat";
...
function toIntervalData(bass, upper) {
...
function simplifyFigures(intervals, forceShowThird = false) {
    ...
    if (has(7)) {
        ...
        if (has(3) && !has(5))
            return alt3 ? [s7, alt3] : [s7, { number: 3 }];
        if (has(5) && !has(3))
            return alt5 ? [s7, alt5] : [s7, { number: 5 }];
        return [s7];
    }
...
function extractDivisions(score) {
...
function parseMeasureNumber(raw, fallback) {
    const n = Number(raw ?? "");
    return Number.isFinite(n) ? n : fallback;
}
function measureDurationDiv(measure) {
...
function buildMeasureStartMap(parts) {
    const durationByMeasure = new Map();
    for (const part of parts) {
        ...
        for (let m = 0; m < measures.length; m += 1) {
            ...
            if (duration > prev)
                durationByMeasure.set(measureNumber, duration);
        }
    }
    const orderedMeasureNumbers = [...durationByMeasure.keys()].sort((a, b) => a - b);
    const startByMeasure = new Map();
    let runningStart = 0;
...
function extractPartEvents(part, measureStartMap) {
    const measures = asArray(part.measure);
    const events = [];
    let absoluteDiv = 0;
    let divisions = 1;
    ...
    for (let m = 0; m < measures.length; m += 1) {
        ...
        for (const note of notes) {
            ...
            if (!isRest && note.pitch) {
                ...
                const tieStart = ties.some((t) => String(t?.["@_type"] ?? "") === "start");
                const tieStop = ties.some((t) => String(t?.["@_type"] ?? "") === "stop");
...
function findBassPartIndex(allPartEvents) {
    return (allPartEvents
        .map((events, index) => ({ index, avg: averageMidi(events) }))
        .sort((a, b) => a.avg - b.avg)[0]?.index ?? 0);
}
function inferFigures(allPartEvents, minDurationDiv) {
    ...
    for (const bass of bassEvents) {
        const changePoints = new Set([bass.start]);
        ...
        const sortedPoints = [allPoints[0]];
        ...
        for (let i = 0; i < sortedPoints.length; i++) {
            ...
            if (figures === prevFigures) {
                if (segs.length > 0)
                    segs[segs.length - 1].durationDiv += segDuration;
            }
            else {
...
function buildFiguredBassXml(slots, durationDiv, indent) {
    const figureLines = slots
        .map((slot) => {
        const lines = [`${indent}  <figure>`];
        if (slot.prefix)
            lines.push(`${indent}    <prefix>${slot.prefix}</prefix>`);
...
function parseFigureSlots(figuredBassXml) {
    const slots = [];
 ...
 */
function collectAndStripExistingFiguredBass(xmlText, bassPartIndex) {
...
function eventSig(slots, durationDiv) {
    return `${slots.map(slotSig).join("|")}@${durationDiv}`;
}
function stripAllFiguredBass(xmlText) {
...
function buildFigureChangeReport(inferred, existing, includeAdditions = true) {
    const inferredMap = new Map();
    const existingMap = new Map();
    for (const fe of inferred) {
        if (fe.slots.length === 0)
            continue;
        if (!inferredMap.has(fe.anchorDiv))
            inferredMap.set(fe.anchorDiv, []);
        inferredMap.get(fe.anchorDiv).push(fe);
    }
    for (const ex of existing) {
        if (ex.slots.length === 0)
            continue;
        if (!existingMap.has(ex.anchorDiv))
            existingMap.set(ex.anchorDiv, []);
 ...
 */
function getBassPartId(xmlText, bassPartIndex) {
...
function getPartIdsInOrder(xmlText) {
    const ids = [];
    const partIdRegex = /<part\s[^>]*id="([^"]+)"/g;
    let match;
    while ((match = partIdRegex.exec(xmlText)) !== null) {
...
function resolveBassPartIndex(allPartEvents, xmlText, requestedBassPartId) {
    ...
    if (idx === -1) {
        ...
        throw new Error(`Requested bass part id "${requestedBassPartId}" not found. Available part ids: ${available}`);
    }
    return idx;
}
/**
 ...
 */
function insertFiguredBass(xmlText, figures, bassPartIndex, allowExisting = false) {
    // Build lookup: bass note anchorDiv -> ordered list of FigureEvents for that note
    const figureMap = new Map();
    for (const fe of figures) {
        if (!figureMap.has(fe.anchorDiv))
            figureMap.set(fe.anchorDiv, []);
        figureMap.get(fe.anchorDiv).push(fe);
    }
    if (!allowExisting && xmlText.includes("<figured-bass>")) {
        throw new Error("Input file already contains <figured-bass> elements. " +
...
function tonicPitchClass(fifths) {
...
function getPartNames(xmlText, numParts) {
    const names = [];
...
function isPerfectOctave(midiA, midiB) {
 ...
 */
function analyzeVoiceLeading(allPartEvents, partNames, bassPartIndex, opts) {
    ...
    const avgMidis = allPartEvents.map((events) => events.length === 0 ? -Infinity : events.reduce((s, n) => s + n.midi, 0) / events.length);
    let sopranoIndex = -1;
    ...
    function resolvesToTonicStepwise(notes, ltIndex, tonicPc) {
        ...
        for (let j = ltIndex + 1; j <= maxIndex; j += 1) {
            ...
            if (n.start === last.start)
                continue;
            if (!isMelodicStep(last, n))
                return false;
            if ((n.midi % 12) === tonicPc)
                return true;
            last = n;
        }
        return false;
    }
    function push(rule, measure, beat, timeDiv, partIndices, voiceNames, detail, noteTargets) {
                    ...
                    if (aAttacked || bAttacked) {
                        ...
                        if (bothMoved) {
                            ...
                            if (similarMotion &&
                                ...
                                push("Parallel P5ths", n.measure, n.beat, n.start, [pi, pj], nameAB, `P5 → P5 (${noteLabel(prevA)}-${noteLabel(prevB)} → ${noteLabel(curA)}-${noteLabel(curB)})`, [
                            ...
                            if (similarMotion &&
                                ...
                                const n = aAttacked ? curA : curB;
                                push("Parallel octaves", n.measure, n.beat, n.start, [pi, pj], nameAB, `P8 → P8 (${noteLabel(prevA)}-${noteLabel(prevB)} → ${noteLabel(curA)}-${noteLabel(curB)})`, [
                                    { partIndex: pi, startDiv: prevA.start },
                                    { partIndex: pj, startDiv: prevB.start },
                                    { partIndex: pi, startDiv: curA.start },
                                    { partIndex: pj, startDiv: curB.start },
                            ...
                            if (similarMotion && prevA.midi === prevB.midi && curA.midi === curB.midi) {
                                const n = aAttacked ? curA : curB;
                                push("Parallel unisons", n.measure, n.beat, n.start, [pi, pj], nameAB, `unison → unison on ${noteLabel(curA)}`, [
                                    { partIndex: pi, startDiv: prevA.start },
                                    { partIndex: pj, startDiv: prevB.start },
                                    { partIndex: pi, startDiv: curA.start },
                                    { partIndex: pj, startDiv: curB.start },
                        ...
                        if (isOuterPair && aAttacked && bAttacked) {
                            ...
                            if (sopranoLeap && similarMotion) {
                                if (isPerfectOctave(soprano.midi, bass.midi)) {
                                    push("Direct octaves", soprano.measure, soprano.beat, soprano.start, [sopranoIndex, bassPartIndex], `${partNames[sopranoIndex]} & ${partNames[bassPartIndex]}`, `outer voices reach P8 by similar motion, soprano leaps (${noteLabel(sopranoPrev)}→${noteLabel(soprano)})`);
                                }
                                else if (isPerfectFifth(soprano.midi, bass.midi, soprano.degreeAbs, bass.degreeAbs)) {
                                    push("Direct 5ths", soprano.measure, soprano.beat, soprano.start, [sopranoIndex, bassPartIndex], `${partNames[sopranoIndex]} & ${partNames[bassPartIndex]}`, `outer voices reach P5 by similar motion, soprano leaps (${noteLabel(sopranoPrev)}→${noteLabel(soprano)})`);
                                }
                            }
                        }
                    }
                    ...
                    if ((aAttacked || bAttacked)) {
                        ...
                        if (landsOnUnison || voiceExchange) {
                            ...
                            if (prevA.midi !== prevB.midi && prevAHigher !== curAHigher) {
                                const n = aAttacked ? curA : curB;
                                push("Voice crossing", n.measure, n.beat, n.start, [pi, pj], nameAB, `${partNames[pi]} crosses ${partNames[pj]} (${noteLabel(curA)} vs ${noteLabel(curB)})`);
                            }
                        }
                    }
                }
    ...
    for (let p = 0; p < numParts; p++) {
        ...
        for (let i = 1; i < notes.length; i++) {
            ...
            if (diatDist % 7 === 1 && chromDist % 12 === 3 && chromDist < 12) {
                push("Augmented 2nd", curr.measure, curr.beat, curr.start, [p], name, `melodic aug 2nd: ${noteLabel(prev)} → ${noteLabel(curr)}`);
            }
            // Rule 8: Melodic tritone leap (diatonic 4th, chromatic 6 = aug 4th / dim 5th)
            if (diatDist % 7 === 3 && chromDist % 12 === 6 && chromDist < 12) {
                push("Melodic tritone", curr.measure, curr.beat, curr.start, [p], name, `tritone leap: ${noteLabel(prev)} → ${noteLabel(curr)}`);
            }
            // Rule 9: Large leap ≥ minor 7th (10 semitones)
            if (chromDist >= 10) {
                push("Large leap", curr.measure, curr.beat, curr.start, [p], name, `leap of ${chromDist} semitone${chromDist !== 1 ? "s" : ""}: ${noteLabel(prev)} → ${noteLabel(curr)}`);
            }
            ...
            if (p !== bassPartIndex) {
                ...
                if ((prev.midi % 12) === leadingPC) {
                    ...
                    if (!resolvedUp && !innerVoiceException && !fugalLineException) {
                        push("Leading tone unresolved", prev.measure, prev.beat, prev.start, [p], name, `LT ${noteLabel(prev)} → ${noteLabel(curr)} (expected tonic ${["C", "C#", "D", "Eb", "E", "F", "F#", "G", "Ab", "A", "Bb", "B"][tonic]})`);
                    }
                }
            }
        }
...
function formatRuleNumberTag(rules) {
    const nums = [...rules]
...
function chooseColorForRules(rules) {
    for (const preferred of RULE_PRIORITY) {
        if (rules.has(preferred))
            return getRuleColor(preferred);
    }
    const first = [...rules][0];
    return getRuleColor(first);
}
function colorVoiceLeadingViolations(xmlText, violations) {
    ...
    for (const v of violations) {
        if (v.noteTargets && v.noteTargets.length > 0) {
            for (const t of v.noteTargets) {
                if (t.partIndex < 0 || t.partIndex >= partIds.length)
                    continue;
        ...
        for (const partIndex of v.partIndices) {
            if (partIndex < 0 || partIndex >= partIds.length)
                continue;
...
function buildLegendDirectionsXml(entries) {
    const lines = [];
    const titleY = 102;
    const gridStartY = 84;
    const rowHeight = 12;
...
function insertLegendIntoFirstMeasure(xmlText, entries) {
    if (entries.length === 0)
        return xmlText;
...
function formatAnalysisReport(violations, inputPath) {
    const header = [
    ...
    for (const v of violations) {
        if (!ruleGroups.has(v.rule))
            ruleGroups.set(v.rule, []);
    ...
    for (const [rule, group] of ruleGroups) {
        ...
        for (const v of group) {
            lines.push(`  m.${String(v.measure).padEnd(4)} beat ${String(v.beat.toFixed(2)).padEnd(6)}` +
